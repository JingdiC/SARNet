...
def get_trainers_td3_group(num_agents, obs_shape_n, action_space, args, num_env, is_train=False):
    ...
    with tf.compat.v1.variable_scope(args.exp_name + "/" + args.adv_test + "_ADV_GROUP", reuse=False):
        for i in range(args.num_adversaries):
            trainers.append(CommAgentTrainerTD3("adv_agent_group", CommActorNetworkTD3, model, obs_ph_n, gru1_ph_n, gru2_ph_n, memory_ph_n, act_ph_n, action_space_n, target_ph, q_gru_ph_n, importance_in_ph, args, i,
                                                num_env, is_train))
    ...
    with tf.compat.v1.variable_scope(args.exp_name + "/" + args.good_test + "_GD_GROUP", reuse=False):
        for i in range(args.num_adversaries, num_agents):
            trainers.append(CommAgentTrainerTD3("good_agent_group", CommActorNetworkTD3, model, obs_ph_n, gru1_ph_n, gru2_ph_n, memory_ph_n, act_ph_n, action_space_n, target_ph, q_gru_ph_n, importance_in_ph, args, i,
                                                num_env, is_train))
...
def load_group_model(num_agents, obs_shape_n, action_space, arglist, num_env, is_train):
